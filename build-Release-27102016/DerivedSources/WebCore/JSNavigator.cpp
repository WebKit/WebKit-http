...
static inline EncodedJSValue jsNavigatorPrototypeFunctionSendBeacon1(ExecState* state)
{
    ...
    if (!state->argument(1).isUndefinedOrNull()) {
        data = JSBlob::toWrapped(state->uncheckedArgument(1));
        if (UNLIKELY(!data))
            return throwArgumentTypeError(*state, 1, "data", "Navigator", "sendBeacon", "Blob");
    }
...
static inline EncodedJSValue jsNavigatorPrototypeFunctionSendBeacon3(ExecState* state)
{
    ...
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
    DOMFormData* data = nullptr;
    if (!state->argument(1).isUndefinedOrNull()) {
        data = JSDOMFormData::toWrapped(state->uncheckedArgument(1));
        if (UNLIKELY(!data))
            return throwArgumentTypeError(*state, 1, "data", "Navigator", "sendBeacon", "DOMFormData");
    }
    JSValue result = jsBoolean(WebCore::NavigatorBeacon::sendBeacon(impl, *context, WTFMove(url), WTFMove(data), ec));
...
EncodedJSValue JSC_HOST_CALL jsNavigatorPrototypeFunctionSendBeacon(ExecState* state)
{
    ...
    if (argsCount == 2) {
        ...
        if (distinguishingArg.isObject() && asObject(distinguishingArg)->inherits(JSDOMFormData::info()))
