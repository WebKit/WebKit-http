...
template<> JSC::EncodedJSValue JSC_HOST_CALL JSDOMFormDataConstructor::construct(JSC::ExecState* exec)
{
    ASSERT(exec);
    return constructJSDOMFormData(*exec);
}
...
static inline EncodedJSValue jsDOMFormDataPrototypeFunctionAppend1(ExecState* state)
{
    ...
    if (UNLIKELY(!castedThis))
        return throwThisTypeError(*state, "FormData", "append");
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSDOMFormData::info());
    auto& impl = castedThis->wrapped();
    if (UNLIKELY(state->argumentCount() < 2))
        return throwVMError(state, createNotEnoughArgumentsError(state));
    auto name = valueToUSVString(state, state->argument(0));
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
    auto value = valueToUSVString(state, state->argument(1));
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
...
static inline EncodedJSValue jsDOMFormDataPrototypeFunctionAppend2(ExecState* state)
{
    ...
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
    auto value = JSBlob::toWrapped(state->argument(1));
    if (UNLIKELY(!value))
        return throwArgumentTypeError(*state, 1, "value", "FormData", "append", "Blob");
