...
template<> EncodedJSValue JSC_HOST_CALL JSNotificationConstructor::construct(ExecState* state)
{
    ...
    auto options = state->argument(1).isUndefined() ? Notification::Options() : convertDictionary<Notification::Options>(*state, state->uncheckedArgument(1));
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
    ...
    auto object = Notification::create(document, WTFMove(title), options.value());
