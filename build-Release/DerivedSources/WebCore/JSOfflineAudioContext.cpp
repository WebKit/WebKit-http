...
template<> EncodedJSValue JSC_HOST_CALL JSOfflineAudioContextConstructor::construct(ExecState* state)
{
    ...
    if (UNLIKELY(!context))
        return throwConstructorDocumentUnavailableError(*state, "webkitOfflineAudioContext");
    auto object = OfflineAudioContext::create(*context, WTFMove(numberOfChannels), WTFMove(numberOfFrames), WTFMove(sampleRate), ec);
    if (UNLIKELY(ec)) {
