...
JSString* jsStringWithCache(ExecState* state, FetchRequest::ReferrerPolicy enumerationValue)
{
    ...
    static_assert(static_cast<size_t>(FetchRequest::ReferrerPolicy::OriginOnly) == 3, "FetchRequest::ReferrerPolicy::OriginOnly is not 3 as expected");
...
template<> Optional<FetchRequest::ReferrerPolicy> parse<FetchRequest::ReferrerPolicy>(ExecState& state, JSValue value)
{
    ...
    if (stringValue == "origin-only")
        return FetchRequest::ReferrerPolicy::OriginOnly;
...
{
    return "\"\", \"no-referrer\", \"no-referrer-when-downgrade\", \"origin-only\", \"origin-when-cross-origin\", \"unsafe-url\"";
}
...
static inline EncodedJSValue constructJSFetchRequest1(ExecState* state)
{
    ...
    if (UNLIKELY(!input))
        return throwArgumentTypeError(*state, 0, "input", "FetchRequest", nullptr, "FetchRequest");
    ...
    auto object = FetchRequest::create(*context, *input, init, ec);
    if (UNLIKELY(ec)) {
