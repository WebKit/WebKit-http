...
template<> HTMLSlotElement::AssignedNodesOptions convert<HTMLSlotElement::AssignedNodesOptions>(ExecState& state, JSValue value)
{
...
EncodedJSValue JSC_HOST_CALL jsHTMLSlotElementPrototypeFunctionAssignedNodes(ExecState* state)
{
    ...
    auto& impl = castedThis->wrapped();
    auto options = state->argument(0).isUndefined() ? HTMLSlotElement::AssignedNodesOptions() : convert<HTMLSlotElement::AssignedNodesOptions>(*state, state->uncheckedArgument(0));
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
    JSValue result = jsArray(state, castedThis->globalObject(), impl.assignedNodes(WTFMove(options)));
    return JSValue::encode(result);
}
