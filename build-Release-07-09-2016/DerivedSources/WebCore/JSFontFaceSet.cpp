...
template<> EncodedJSValue JSC_HOST_CALL JSFontFaceSetConstructor::construct(ExecState* state)
{
    ...
    auto initialFaces = (toRefPtrNativeArray<FontFace, JSFontFace>(state, state->argument(0), &JSFontFace::toWrapped));
    ...
    auto object = FontFaceSet::create(document, initialFaces);
...
void JSFontFaceSetPrototype::finishCreation(VM& vm)
{
    ...
    putDirect(vm, vm.propertyNames->iteratorSymbol, JSFunction::create(vm, globalObject(), 0, ASCIILiteral("[Symbol.Iterator]"), jsFontFaceSetPrototypeFunctionSymbolIterator), ReadOnly | DontEnum);
}
