...
static inline EncodedJSValue jsIDBIndexPrototypeFunctionOpenKeyCursor1(ExecState* state)
{
    ...
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
    JSValue result = toJS(state, castedThis->globalObject(), impl.openKeyCursor(*context, WTFMove(range), WTFMove(direction), ec));
...
static inline EncodedJSValue jsIDBIndexPrototypeFunctionOpenKeyCursor2(ExecState* state)
{
    ...
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
    JSValue result = toJS(state, castedThis->globalObject(), impl.openKeyCursor(*context, WTFMove(key), WTFMove(direction), ec));
...
static inline EncodedJSValue jsIDBIndexPrototypeFunctionGetKey1(ExecState* state)
{
    ...
    if (!context)
        return JSValue::encode(jsUndefined());
    auto key = JSIDBKeyRange::toWrapped(state->argument(0));
    JSValue result = toJS(state, castedThis->globalObject(), impl.getKey(*context, WTFMove(key), ec));
