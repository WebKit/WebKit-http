...
JSC::EncodedJSValue JSC_HOST_CALL jsDocumentPrototypeFunctionDefineElement(JSC::ExecState*);
#endif
...
#if ENABLE(CUSTOM_ELEMENTS)
    { "defineElement", JSC::Function, NoIntrinsic, { (intptr_t)static_cast<NativeFunction>(jsDocumentPrototypeFunctionDefineElement), (intptr_t) (2) } },
#else
    { 0, 0, NoIntrinsic, { 0, 0 } },
#endif
...
void JSDocumentPrototype::finishCreation(VM& vm)
{
    ...
    if (!RuntimeEnabledFeatures::sharedFeatures().customElementsEnabled()) {
        Identifier propertyName = Identifier::fromString(&vm, reinterpret_cast<const LChar*>("defineElement"), strlen("defineElement"));
...
EncodedJSValue jsDocumentDefaultCharset(ExecState* state, EncodedJSValue thisValue, PropertyName)
{
    ...
    auto& impl = castedThis->wrapped();
    JSValue result = jsStringWithCache(state, impl.defaultCharset());
    return JSValue::encode(result);
}
...
EncodedJSValue JSC_HOST_CALL jsDocumentPrototypeFunctionCreateTouch(ExecState* state)
{
    ...
    auto& impl = castedThis->wrapped();
    ExceptionCode ec = 0;
    auto window = JSDOMWindow::toWrapped(*state, state->argument(0));
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
    auto target = JSEventTarget::toWrapped(*state, state->argument(1));
    if (UNLIKELY(state->hadException()))
        return JSValue::encode(jsUndefined());
...
#if ENABLE(CUSTOM_ELEMENTS)
EncodedJSValue JSC_HOST_CALL jsDocumentPrototypeFunctionDefineElement(ExecState* state)
{
    JSValue thisValue = state->thisValue();
    auto castedThis = jsDocumentCast(thisValue);
    if (UNLIKELY(!castedThis))
        return throwThisTypeError(*state, "Document", "defineElement");
    ASSERT_GC_OBJECT_INHERITS(castedThis, JSDocument::info());
    return JSValue::encode(castedThis->defineElement(*state));
}
